...
enum OneTouchPassingCoachState: Equatable {
    case ready
...
struct OneTouchPassingCoachRemoteView: View {
        ...
        VStack(spacing: 20) {
            if !coachSessionConnected, timedSession.isSessionActive {
                CoachRemoteWaitingForDisplaySessionView(
                    headerTitle: "Coach — \(ActivityKind.oneTouchPassing.displayName)",
    ...
    private func sharedSessionInput(repIndex: Int) -> some View {
        CoachSessionView(
            coachRemoteHeaderTitle: "Coach — \(ActivityKind.oneTouchPassing.displayName)",
    ...
    @discardableResult
    private func startNextRep(repIndex: Int) -> Bool {
        ...
        if TimedSessionDisplayIntegration.coachRemoteLoopsEngineChunks {
            repToStart = TimedSessionDisplayIntegration.coachRemoteWrapRepIndex(
                repIndex,
                chunkSize: totalReps
            )
        } else {
            guard repIndex < totalReps else { return false }
            repToStart = repIndex
        }
        ...
        #if DEBUG
        if Self.partnerTransportMode == .relayWebSocket {
            otpCoachRelayLog("send nextRep repIndex=\(repToStart)")
        }
        #endif
        remoteService.sendNextRep(repIndex: repToStart)
        currentRepIndex = repToStart
        state = .waitingForRepStart(repIndex: repToStart)
        scheduleRepStartAckTimeout(for: repToStart)
        return true
    }
    ...
    private func advanceCoachRepIndexAfterPass(completedRepIndex: Int) {
        ...
        CoachFirstRunGuidanceStore.markCompletedFirstRun(activityId: ActivityKind.oneTouchPassing.sessionActivityActivityId)
        state = .blockComplete
        pendingDirectionLogRepZeroBased = nil
    }
    ...
    private func logExit(repIndex: Int, gate: Gate) {
        guard CoachRepFlowDecoupling.mayLogDirectionForRep(
            ...
            repIndex: repIndex
        ), repIndex < totalReps else { return }
        #if DEBUG
        if Self.partnerTransportMode == .relayWebSocket {
    ...
    private func handleRepStartedAcknowledgement(repIndex: Int) {
        if case .waitingForRepStart(let waitingRep) = state, waitingRep == repIndex {
            ...
            print("[OTP Coach] repStarted ACK in \(deltaMs)ms for rep \(repIndex)")
            #endif
            return
        }
        ...
        if case .ready = state, currentRepIndex == repIndex {
            ...
            state = .armedForPass(repIndex: repIndex)
            #if DEBUG
            print("[OTP Coach] repStarted ACK accepted while .ready (late after timeout) rep \(repIndex)")
            #endif
            return
        }
        ...
        #if DEBUG
        let deltaMs = Int((Date().timeIntervalSince1970 - repStartTapSentTimestamp) * 1000)
        print("[OTP Coach] late repStarted ACK accepted in \(deltaMs)ms for rep \(repIndex)")
        #endif
    }
    ...
    private func handleBeepArmed(repIndex: Int) {
        guard repIndex == currentRepIndex else {
            #if DEBUG
            print("[OTP Coach] beepArmed ignored: repIndex=\(repIndex) currentRepIndex=\(currentRepIndex)")
            #endif
            return
        }
